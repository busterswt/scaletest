...
class Pinger(object):
    ...
    def ping(self, ip, build_duration):
        ...
        keepPinging = True
        while keepPinging:
            ret = subprocess.Popen(['ip','netns','exec','qdhcp-45348a1d-3e9e-47f7-b217-db8297faad45','ping', '-c', '1', '-w', '1', ip], stdout=subprocess.PIPE)
...
def launch(hostname,network):
    ...
    try:
        ...
        return _base_info['primary_address'],server,boot_start,_base_info['primary_port']
    except Exception:
...
#    ping.hosts = ['127.0.0.1', '8.8.8.8', '192.168.0.4', '192.168.0.5','192.168.0.6']
    ...
    floating_network_id = "45348a1d-3e9e-47f7-b217-db8297faad45"

    for i in range(int(sys.argv[1])):    
        # Boot an instance. The IP will be returned.
        try:
            host_addr,server,build_start,port_id = launch(novalib.random_server_name(),sys.argv[2])
            floatingip = neutronlib.create_floatingip(floating_network_id,port_id)
        except:
